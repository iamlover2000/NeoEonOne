	...
	if($_POST['id'] == "" || $_POST['pass'] == ""){
		...
		$member = $pdo->query($sql)->fetch();
		dump($member);
		// die();
		$hash_pw = password_hash($member['pass'],PASSWORD_DEFAULT); //$hash_pw에 POSt로 받아온 아이디열의 비밀번호를 저장합니다. 
		...
		if(password_verify($password, $hash_pw)) //만약 password변수와 hash_pw변수가 같다면 세션값을 저장하고 알림창을 띄운후 main.php파일로 넘어갑니다.
		{
			...
			echo "<script>alert('로그인되었습니다.'); location.href='./mainp.html';</script>";
